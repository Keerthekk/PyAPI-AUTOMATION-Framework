...
def post_request(url, auth, headers, payload, in_json):
    post_response_data = requests.post(url, headers=headers, auth=(auth), json = payload)
    if in_json is True:
        return post_response_data.json()
    return post_response_data

def patch_request(url, auth, headers, payload, in_json):
    patch_response_data = requests.patch(url,headers=headers, auth=(auth), json = payload)
    if in_json is True:
        return patch_response_data.json()
    return patch_response_data

def delete_request(url, auth, header, in_json):
    delete_response_data = requests.delete(url, headers=header, auth=(auth))
    if in_json is True:
